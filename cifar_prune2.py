...
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--weight-decay', '--wd', default=5.0e-4, type=float,
...
def get_momentum_for_weight( weight, optimizer):
    ...
    return grad
def momentum_redistribution( weight, mask, optimizer):
...
# -------------------------------------------------------------
    ...
    for module in model.modules():
        if isinstance(module, nn.Conv2d):
            ...
            for curname, weight in module.named_parameters():
                ...
                masks[name][:] = (torch.rand(weight.shape) < args.density).float().data.cuda()
                
                curmask = masks[name]

                # redistribution
                name2variance[name] = momentum_redistribution( weight, curmask,optimizer)
    ...
    for name in name2variance:
        ...
        percent_pruned_actual = args.percent * (max_ - name2variance[name]) / (max_ - min_)
